...
const BlogDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`https://skinmusebackend-delta.vercel.app/api/blogs/${id}`);
        setBlog(res.data.blog);

        const allRes = await axios.get(`https://skinmusebackend-delta.vercel.app/api/blogs`);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        {/* Back Button */}
        <Button variant="ghost" onClick={() => navigate('/blogs')} className="mb-6">
        ...
        {/* Article Content */}
        <div className="prose prose-gray max-w-none" dangerouslySetInnerHTML={{ __html: blog.content }} />
