...
export default function EditProductForm() {
  ...
  // ✅ Get selected category title from _id
  ...
  // ✅ Fetch product — runs after allCategories is ready
  useEffect(() => {
    if (!productId || allCategories.length === 0) return;
      ...
      try {
        ...
        if (categoryId) {
          const subs = allCategories.filter(
            (cat: any) =>
              cat.parent_id?._id === categoryId || cat.parent_id === categoryId
          );
        ...
        setFormData({
          ...
          colors: product.colors
            ...
            : [],
          litres: product.litres
            ? product.litres.map((l: any) => ({
                amount: l.amount,
                stock: l.stock?.toString() || "0",
                price: l.price || 0,
              }))
            : [],
        ...
        if (product.images?.length > 0) {
          setAllImages(
            product.images.map((url: string) => ({ preview: url, isExisting: true }))
          );
        }
      } catch {
        toast.error("Failed to fetch product data!");
    ...
    fetchProduct();
  }, [productId, allCategories]); // ✅ depends on allCategories being loaded

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
  };

  // ✅ When main category changes → filter subcategories
  const handleCategoryChange = (value: string) => {
    setFormData({ ...formData, category: value, subCategoryId: "", colors: [], litres: [] });
    const subs = allCategories.filter(
      (cat: any) => cat.parent_id?._id === value || cat.parent_id === value
    );
    setSubCategories(subs);
  };
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* ✅ Subcategory — shows only when subcategories exist */}
                    ...
                    onChange={(e) => {
                      setFormData({
                        ...
                        litres: e.target.checked
                          ? formData.litres.length
                            ? formData.litres
                            : [{ amount: "", stock: "0", price: 0 }]
                          : [],
                      });
                    }}
                  />
